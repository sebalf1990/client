...
Public Sub AddOrUpdateEffect(ByRef EffectList As t_ActiveEffectList, ByRef Effect As t_ActiveEffect)
    ...
    If Not IsArrayInitialized(EffectList.EffectList) Then
        ReDim EffectList.EffectList(ACTIVE_EFFECT_LIST_SIZE) As t_ActiveEffect
    ElseIf EffectList.EffectCount >= UBound(EffectList.EffectList) Then
        ReDim Preserve EffectList.EffectList(EffectList.EffectCount * 1.2) As t_ActiveEffect
    End If
    EffectList.EffectList(EffectList.EffectCount) = Effect
...
Public Function FindEffectIndex(ByRef EffectList As t_ActiveEffectList, ByRef Effect As t_ActiveEffect) As Integer
    ...
    For i = 0 To EffectList.EffectCount - 1
        If EffectList.EffectList(i).TypeId = Effect.TypeId And EffectList.EffectList(i).id = Effect.id Then
            FindEffectIndex = i
            Exit Function
        End If
    Next i
