...
Sub SwitchMap(ByVal map As Integer, Optional ByVal NewResourceMap As Integer = 0)
    ...
    If Not SurfaceDB Is Nothing Then
        ...
        If usedMB >= TexHighWaterMark Then
            ' Optional debug log
            Debug.Print "Texture memory " & format$(usedMB, "0.00") & " MB = " & TexHighWaterMark & " MB ? releasing " & NumTexRelease & " textures"
...
Function HayAgua(ByVal x As Integer, ByVal y As Integer) As Boolean
    ...
    With MapData(x, y).Graphic(1)
        HayAgua = (.GrhIndex >= 1505 And .GrhIndex <= 1520) Or (.GrhIndex >= 124 And .GrhIndex <= 139) Or (.GrhIndex >= 24223 And .GrhIndex <= 24238) Or (.GrhIndex >= 24303 And _
                .GrhIndex <= 24318) Or (.GrhIndex >= 468 And .GrhIndex <= 483) Or (.GrhIndex >= 44668 And .GrhIndex <= 44683) Or (.GrhIndex >= 24143 And .GrhIndex <= 24158) Or ( _
                .GrhIndex >= 12628 And .GrhIndex <= 12643) Or (.GrhIndex >= 2948 And .GrhIndex <= 2963)
    End With
    Exit Function
HayAgua_Err:
...
Public Sub Draw_Sombra(ByRef Grh As Grh, _
