...
Public Sub RenderMinimapCentered(ByVal currentMap As Integer, ByVal tileX As Integer, ByVal tileY As Integer, Optional ByVal viewDeltaW As Long = 0, Optional ByVal viewDeltaH As Long = 0)
    ...
    If (lastWorld <> worldNum) Or (worldBitmap Is Nothing) Then
        Select Case worldNum
            Case 1
                Set worldBitmap = LoadInterface("mapa1.bmp", False)
            Case 2
                Set worldBitmap = LoadInterface("mapa2.bmp", False)
            Case 3
                Set worldBitmap = LoadInterface("mapa3.bmp", False)
            Case Else
    ...
    If (worldBitmap Is Nothing) Then
        Set frmMain.MiniMap.Picture = LoadMinimap(currentMap)
        Exit Sub
    End If
