...
Public Sub TranslateCharacterToPos(ByVal charindex As Integer, ByVal NewX As Integer, ByVal NewY As Integer, ByVal TranslationTime As Long)
    ...
    With charlist(charindex)
        ...
        .scrollDirectionX = Sgn(DiffX)
        .scrollDirectionY = Sgn(DiffY)
        If (NewY < MinLimiteY) Or (NewY > MaxLimiteY) Or (NewX < MinLimiteX) Or (NewX > MaxLimiteX) Then
            Call EraseChar(charindex)
        End If
        .Moving = False
...
Public Sub Char_Dialog_Set(ByVal char_index As Integer, _
                           ByVal char_dialog As String, _
                           ByVal char_dialog_color As Long, _
                           ByVal char_dialog_life As Byte, _
                           ByVal Sube As Byte, _
                           Optional ByVal font_index As Integer = 1, _
                           Optional ByVal IsSpell As Boolean = False, _
                           Optional ByVal MinChatTime As Integer = 0, _
                           Optional ByVal MaxChatTime As Integer = 0)
